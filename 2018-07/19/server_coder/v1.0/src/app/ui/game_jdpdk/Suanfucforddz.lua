...
local Card_Type_Weight = {
	...
	[poker_common_pb.EN_POKER_TYPE_BOMB_OF_3A] = 2,
}
...
function Suanfucforddz:reconsitutionData(data)
	...
	for i,v in ipairs(data) do
		local weight = self:getCardWeight(v)
...
function Suanfucforddz:getUsableCardType(typ)
	local usabletype = self:getUpperCardType(typ)
...
function Suanfucforddz:commonCompare(lastgroup,curgroup)
	local uppertype = self:getUpperCardType(lastgroup.type)
	...
	-- 检验是否为同级牌型
	if self:getIsEquativeType(lastgroup,curgroup) then
...
function Suanfucforddz:comparetype(lastaction,groups)
	...
	for i,v in ipairs(groups) do
		if self:commonCompare(lastaction,v) then
			table.insert(output,v)
		end
	end
...
function Suanfucforddz:getIsStraight2(data,totalnum)
	...
	if self.gamescene:getTableConf().able_two_lain_dui then
		minnum = 4
	end

	local output = {}
	if totalnum >= minnum and #data == totalnum/2 then
		local begin = data[1].weight
		local ed = data[#data].weight
		if ed <= 14 and (ed - begin == (#data - 1)) then
			for i,v in ipairs(data) do
...
function Suanfucforddz:getIsStraight3_2(data,totalnum)
	...
	if totalnum >= 10 and totalnum%5 == 0 then
		...
		for i,v in ipairs(data) do
			if data[i].weight <= 14 and data[i+totalnum/5-1] and (data[i].weight - data[i+totalnum/5-1].weight == totalnum/5 - 1) then
				table.insert(output,{type = poker_common_pb.EN_POKER_TYPE_STRAIGHT_3_2,real = data[i+totalnum/5-1].weight,num = totalnum/5})
				return output
			end
		end 
	end
...
function Suanfucforddz:getIsQuadrupleWithThree(data,totalnum)
	local output = {}

	if totalnum == 7 then
		for i,v in ipairs(data) do
			if v.num == 4 then
				table.insert(output,{type = poker_common_pb.EN_POKER_TYPE_QUADRUPLE_WITH_THREE,real = v.weight,num = 1})
				break
			end
		end
	end
...
function Suanfucforddz:getIs3ABomb(data,totalnum)
	local output = {}

	if totalnum == 3 then
		for i,v in ipairs(data) do
			if v.num == 3 and v.weight == 14 then
				table.insert(output,{type = poker_common_pb.EN_POKER_TYPE_BOMB_OF_3A,real = v.weight,num = 1})
				break
			end
		end
	end
...
function Suanfucforddz:getype(data,isall)
	...
	-- 重构数据
	local redata = self:reconsitutionData(data)

	local usabletable = {}

	self:insertDataToTable(usabletable,self:getIsSingle(redata,total))

	self:insertDataToTable(usabletable,self:getIsPair(redata,total))

	if isall then
		self:insertDataToTable(usabletable,self:getisTriple(redata,total))
		self:insertDataToTable(usabletable,self:getisTripleWithOne(redata,total))
	end

	self:insertDataToTable(usabletable,self:getisTripleWithTwo(redata,total))

	self:insertDataToTable(usabletable,self:getIsBomb(redata,total))

	-- self:insertDataToTable(usabletable,self:getIsGunlongBomb(redata,total))

	if self.gamescene:getTableConf().bomb_3_a then
		self:insertDataToTable(usabletable,self:getIs3ABomb(redata,total))
	end

	self:insertDataToTable(usabletable,self:getIsStraight(redata,total))

	self:insertDataToTable(usabletable,self:getIsStraight2(redata,total))

	self:insertDataToTable(usabletable,self:getIsStraight3_2(redata,total))

	self:insertDataToTable(usabletable,self:getIsQuadrupleWithThree(redata,total))
...
function Suanfucforddz:getAllBomb(data)
	...
	if self.gamescene:getTableConf().bomb_3_a then
		...
		for i,v in ipairs(data) do
			if v.num >= 3 and v.weight == 14 then
				table.insert(output,{type = poker_common_pb.EN_POKER_TYPE_BOMB_OF_3A,real = v.weight,values = {v.values[1],v.values[2],v.values[3]},num = 1})
				break
			end
		end
	end
...
function Suanfucforddz:getAllQuadrupleWithThree(data)
	local origindata = clone(data)
	data = clone(data)
	table.sort(data,function (a,b)
		return a.weight < b.weight
	...
	for i,v in ipairs(data) do
		...
		if v.num == 4 then
			...
			local weights = {}
			for m=i,4 do
				table.insert(values,v.values[1])
				table.insert(values,v.values[2])
				table.insert(values,v.values[3])
				table.insert(values,v.values[4])
				-- table.insert(weights,v.weight)
			end

			-- table.insert(output,{type = poker_common_pb.EN_POKER_TYPE_STRAIGHT_3,real = v.weight,values = values})
			table.insert(maintable,{values = values,weight = v.weight})
		end
		-- end
	end
			...
			if not issame then
				for _i=1,n.num do
					...
					if #attachtable == 3 then
						...
						table.insert(output,{type = poker_common_pb.EN_POKER_TYPE_QUADRUPLE_WITH_THREE,real = v.weight,values = values,num = 1})
						break
					end
				end
			end
...
function Suanfucforddz:gettips(data,action)
	...
	local usabletypetable = self:getUsableCardType(action.type)
	...
	for i,v in ipairs(usabletypetable) do
		if v == poker_common_pb.EN_POKER_TYPE_SINGLE_CARD then
			self:insertDataToTable(output,self:getAllSingle(redata))
		elseif v == poker_common_pb.EN_POKER_TYPE_PAIR then
			self:insertDataToTable(output,self:getAllPair(redata))
		elseif v == poker_common_pb.EN_POKER_TYPE_TRIPLE_WITH_TWO then
			self:insertDataToTable(output,self:getAllTripleWithTwo(redata))
		elseif v == poker_common_pb.EN_POKER_TYPE_STRAIGHT then
			self:insertDataToTable(output,self:getAllStraight(redata,action.num))
		elseif v == poker_common_pb.EN_POKER_TYPE_STRAIGHT_2 then
			self:insertDataToTable(output,self:getAllStraight2(redata,action.num))
		elseif v == poker_common_pb.EN_POKER_TYPE_STRAIGHT_3_2 then
			self:insertDataToTable(output,self:getAllStraight3_2(redata,action.num))
		elseif v == poker_common_pb.EN_POKER_TYPE_QUADRUPLE_WITH_THREE then
			self:insertDataToTable(output,self:getAllQuadrupleWithThree(redata,action.num))
		elseif v == poker_common_pb.EN_POKER_TYPE_BOMB then
			self:insertDataToTable(output,self:getAllBomb(redata,action))
		end
	end
	...
	for i=#output,1,-1 do
		if not self:commonCompare(action,output[i]) then
			table.remove(output,i)
		end
	end
...
function Suanfucforddz:getrealcardsTab(cards,action)
	...
	elseif action.type == poker_common_pb.EN_POKER_TYPE_BOMB_OF_3A then
		for j=1,3 do
			table.insert(cardslist,{value = realtable[action.real].values[j],islai =false})
		end
	elseif action.type == poker_common_pb.EN_POKER_TYPE_QUADRUPLE_WITH_THREE then
		local laiindex = 0
		for j=1,4 do
			table.insert(cardslist,{value = realtable[action.real].values[j],islai =false})
		end
...
function Suanfucforddz:selectStraight(pokers,handcards)
	...
	while (index<=#pokers) do
		...
		if self:getrealvalue(pokers[index]:getPoker()) < 15 then
			if index == 1 then
