...
function TestCard:initView()
	self.widget = cc.CSLoader:createNode("ui/game_mj/testcard.csb")
...
function TestCard:initDeck()
	local deckcard = {}
    local _list = { 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9,
...
function TestCard:refreshScrollview(scrollview,typ)
	...
	for i,v in ipairs(scrollview.data) do
        -- print(".........v = ",v)
		local pokerCard = MaJiangCard.new(MAJIANGCARDTYPE.BOTTOM,v)
            :addTo(scrollview)
            :setPosition(cc.p((i-1)*50 + 79*SCALE - 50,scrollview:getContentSize().height/2))
            :setScale(SCALE)
        if typ then
...
function TestCard:addCardEvent2(tile)
    ...
    local function onTouchBegan(touch, event)
        ...
        local rect = cc.rect(0, 0, s.width, s.height)
        if cc.rectContainsPoint(rect,locationInNode) then
            moveCard = MaJiangCard.new(MAJIANGCARDTYPE.BOTTOM, tile:getCardValue())
                :addTo(self)
                :setScale(SCALE)
...
function TestCard:readPaidui()
    local str = self:readFile("paidui_mj")
    print("Load-----------------------",str)
    printTable(str,"xp65")
    local cards = cjson.decode(str) or {}
