...
function Socketapi.login(typ,uuid)
    ...
    -- 游客登录   
        ...
        if device.platform == "ios" then
            ...
            -- request.account = "gzp_0001"
            -- request.account = "mj_0002222"
            -- request.account = "huapai_10086" --测试亲友圈
        elseif  device.platform == "android" then
            request.account = LocalData_instance:getopenUDID()
            -- request.account = uuid
        else
            request.account = uuid or LocalData_instance:getopenUDID()
            -- request.account = "gzp_001"
            -- request.account = "gzp_002"
            -- request.account = "gzp_003"
            -- request.account = "gzp_000004"
            -- request.account = "huapai_100861" --测试亲友圈
            -- request.account = "TEST_07"
        end
    end 
    ...
    -- request.token = "5a054c102b7303c80b566d2640fe5952"
    -- request.account = "oh7AXt2NEINo6laAER_0aomyM81Q"
    -- request.acc_type = 10
    --request.account = "oooooo3"
        ...
        if k ~= "ztype" and k ~= "ttype" then
            if type(v) == "table" then
                for ii,vv in ipairs(v) do
                    request.conf[k]:append(tonumber(vv))
                end
            else
                request.conf[k] = v 
            end 
...
function Socketapi.requestCreateTableForFree(tab)
    print("Socketapi.requestCreateTable")
...
function Socketapi.requestQuickMatch(matchid)
    local msg = poker_msg_pb.PBCSMsg()
    local request = msg.cs_request_quick_match
    request.match_session_id = tonumber(matchid)

    SocketConnect_instance:send("cs_request_quick_match", msg)
end

function Socketapi.requestEnterMatch(tid,game_svid,postype)
    print("Socketapi.requestEnterTable == ",tid)
    local msg = poker_msg_pb.PBCSMsg()
    local request = msg.cs_request_sdr_enter_table
    request.tid = tonumber(tid)
    request.game_svid = game_svid
...
function Socketapi.requestLeaveMatch()
    local msg = poker_msg_pb.PBCSMsg()
    local request = msg.cs_request_leave_match
    request.is_leave = true
    SocketConnect_instance:send("cs_request_leave_match", msg)
end

function Socketapi.requestChangeTuoGuanState(bool)
    local msg = poker_msg_pb.PBCSMsg()
    local request = msg.cs_request_change_tuo_guan_state
    request.tuo_guan_state = bool
    SocketConnect_instance:send("cs_request_change_tuo_guan_state", msg)
end
...
-- 操作类型
function Socketapi.request_do_action(seat_index,act_type,dest_card,cards,token,choice_token,isJian)
    local msg = poker_msg_pb.PBCSMsg()
...
--茶馆锁定玩法
-- CSRequestChangeTeabarCreateInfo  = 0x8033;//请求录入茶馆创建房间信息
-- CSResponseChangeTeabarCreateInfo cs_response_change_teabar_create_info = 0x8034;//回复录入茶馆创建房间信息
function Socketapi.cs_request_change_teabar_create_info(_teaid,_type,_str)
    local msg = poker_msg_pb.PBCSMsg()
    local request = msg.cs_request_change_teabar_create_info
    request.tbid = _teaid
    request.game_type = _type
    request.teabar_create_info = _str
    SocketConnect_instance:send("cs_request_change_teabar_create_info", msg)
end
------------club----------