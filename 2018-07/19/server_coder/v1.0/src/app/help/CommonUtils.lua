...
function CommonUtils.shareclub(tbid,name,info)
    ...
    local title =  "亲友圈招募令 点此加入"
    local info = "我的亲友圈【"..name.."】ID".."【"..tbid.."】".."介绍:"..info
...
function CommonUtils.sharerecord(recordid)
    ...
    local info = "我分享了一个【今日花牌】牌局记录，大家帮我看看。"
...
function CommonUtils.fangchengmi()
    ...
    local localtime = cc.UserDefault:getInstance():getIntegerForKey("fangchengmi",0)
    local newtime = os.time()
    local day1 = os.date("%j", newtime)
    local day2= os.date("%j", localtime)
    if day1 == day2 then
        print("时间："..(newtime - localtime))
        if (newtime - localtime) >(3600*3) then

            LaypopManger_instance:PopBox("PromptBoxView",1,{tipstr = "根据《网络游戏未成年人防沉迷系统》中条款规定，您当前在游戏牌局内累计时间已满3个小时了噢。请注意游戏时间，勿要沉迷！",sureCallFunc = func})    
            cc.UserDefault:getInstance():setIntegerForKey("fangchengmi",newtime)
        end
    else
        cc.UserDefault:getInstance():setIntegerForKey("fangchengmi",newtime)
    end
...
local guanjianzitable = {"元","角","分","块","人民币","逃单","钱","现金","结算","红包","转账","结账",}
function CommonUtils.checkstr(str)

    for i,v in ipairs(guanjianzitable) do
...
function CommonUtils.checkchange(str)
    if str ~= nil then
        for i,v in ipairs(guanjianzitable) do
