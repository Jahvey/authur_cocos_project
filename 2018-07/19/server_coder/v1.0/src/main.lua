...
function __G__TRACKBACK__(errorMessage)
    ...
    if CONFIG_DEBUG then
        local errorlabel = cc.Label:createWithSystemFont(tostring(errorMessage)..debug.traceback(), "", 30, cc.size(600, 700), cc.TEXT_ALIGNMENT_CENTER,cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
...
function __G__TRACKBACK__RED(errorMessage)

    local _str = errorMessage.."\n"..errorMessage.."\n"..errorMessage.."\n"..errorMessage
    local errorlabel = cc.Label:createWithSystemFont(_str, "", 30, cc.size(600, 700), cc.TEXT_ALIGNMENT_CENTER,cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
    errorlabel:setPosition(display.width/2,display.height/2)
    errorlabel:setColor(cc.c3b(255, 0, 0))
    if cc.Director:getInstance():getNotificationNode() == nil then
        cc.Director:getInstance():setNotificationNode(cc.Node:create())
...
local function main()
    LOCALANDROIDPATH = "com/jiuyouqipahejii/arthur/wxapi/WXEntryActivity"
