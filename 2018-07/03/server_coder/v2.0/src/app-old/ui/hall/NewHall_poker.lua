...
function HallView:initView()
	local widget 
	if #CM_INSTANCE:getMAJIANGGAMESLIST() == 0 then --只有花牌和斗地主
		widget = cc.CSLoader:createNode("ui/newhall/hallView_puke_2.csb")
	else
		widget = cc.CSLoader:createNode("ui/newhall/hallView_puke.csb")
	end	
...
function HallView:initTopNode()
	...
	WidgetUtils.addClickEvent(self.headBg, function( )
		...
		-- LaypopManger_instance:PopBox("DDZSingleResultView")
...
function HallView:refreshCreateBtn()
	...
	if table_id  and isPoker == false and isMaJiang == false  then
	...
	local text1 = createRoom_poker:getChildByName("text")
	local text2 = createRoom_poker:getChildByName("text2")

	if table_id and isPoker == true and isMaJiang == false then
		text1:setVisible(false)
		text2:setVisible(true)
	else
		text2:setVisible(false)
	...
	if table_id and isPoker == false and isMaJiang == true then
		text1:setVisible(false)
		text2:setVisible(true)
	else
		text2:setVisible(false)
