...
function Notinode:responselogin(data)
	...
	if data.result == 0 then
		...
		if data.user then
			...
			if data.user.pos.pos_type >= poker_common_pb.EN_Position_Fpf then
				print("---------------1")
	...
	elseif 	data.result == 100 then
		if glApp.sceneName == "LoginScene" then
	...
	if data.result == 0 then
		...
		if glApp:getCurScene().sceneName ~= "MainScene" then
			...
			elseif data.result == poker_common_pb.EN_MESSAGE_NOT_IN_TEA_BAR then
				func()
				LaypopManger_instance:PopBox("PromptBoxView",1,{tipstr = "无法进入，您不是该茶馆成员"})
			else
