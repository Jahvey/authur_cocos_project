...
function InviteCodeView:initView()
	self.widget = cc.CSLoader:createNode("ui/popbox/bindingCodeView.csb")
	...
	-- 数字node
	self.lineNode = self.mainLayer:getChildByName("lineNode")
	-- 数字
...
function InviteCodeView:confirmBtnCall()
	...
	-- print("========code =",code)
	ComHttp.httpPOST(ComHttp.URL.INVITECODE,{uid = LocalData_instance:getUid(),code = code},function(data)
		if not WidgetUtils:nodeIsExist(self) then
			return
		end
		if data.serverstatus == 1 then
			LaypopManger_instance:PopBox("PromptBoxView",1,{tipstr = "绑定成功",sureCallFunc = function() 
				LaypopManger_instance:back()
			end})
		end
	end)
