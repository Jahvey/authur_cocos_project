...
function SingleResultView:initData(data,owner,opentype)
	release_print("............SingleResultView 1 正常结算 2 回放结算")
	printTable(data,"xp80")
...
function SingleResultView:initView()
	...
	if  self.gamescene then
		...
		local roomidStr = "房间ID:"..self.gamescene:getTableID()
          if self.gamescene:getTableConf().is_free_game then
               roomidStr = "试玩场"
          end
	...
	for k,v in pairs(self.data.players) do
		...
		self:createTips(item,v)
          -- if k == 3 then
               ...
               -- if data.hand_cards_info
               -- print("第4个玩家的摆")
               -- printTable(v.out_cols,"xp23")
          -- end
	...
	local hu_num = fenNode:getChildByName("hu_num")
     ...
     if  self.gamescene then
         ...
         -- qiang_num:setVisible(true)
          ...
          elseif  self.gamescene:getTableConf().ttype == HPGAMETYPE.YCXZP then
               hu_text:setString("金数")
          elseif self.gamescene:getTableConf().ttype == HPGAMETYPE.XCCH then
               qiang_text:setVisible(false)
               qiang_num:setVisible(false)
          elseif  self.gamescene:getTableConf().ttype == HPGAMETYPE.TCGZP then
               hu_text:setString("个子数")
               qiang_text:setString("花牌数")
          elseif  self.gamescene:getTableConf().ttype == HPGAMETYPE.LJSDR then
               qiang_text:setString("番数")
          ...
          elseif  self.gamescene:getTableConf().ttype == HPGAMETYPE.FJSDR then
               qiang_text:setString("档数")
          end
     end
end
...
local tipsListCof = 
{
	...
	[28]={sort = 3,img = "game/result/icon_chengdui.png"},	--28,成对
     ...
     [100] = {sort = 2,img = "game/result/icon_heikasan.png"},  --100,黑卡三
     [101] = {sort = 2,img = "game/result/icon_kasan.png"},  --101,卡三
     [102] = {sort = 2,img = "game/result/icon_shijin.png"},  --102,十金
     [103] = {sort = 2,img = "game/result/icon_heishijin.png"},  --103,黑十金
     [104] = {sort = 2,img = "game/result/icon_hongchonghu.png"},  --104,红冲
     [105] = {sort = 2,img = "game/result/icon_heihuchong.png"},  --105,黑冲

     [124] = {sort = 2,img = "game/result/icon_shiduihu.png"},  --116,十对胡
     [125] = {sort = 2,img = "game/result/icon_heiyise.png"},  --117，黑一色
}
...
function SingleResultView:createTips(item,data)
	...
	local wininfo = data.win_info and data.win_info.styles or {}
     if self.gamescene then
          if  self.gamescene:getTableConf().ttype == HPGAMETYPE.YCXZP then
               tipsListCof[50] = {sort = 2,img = "game/result/icon_quanhei.png"}  --50,全黑
          end
     end
	...
	local _width = 0
     ...
     if self.gamescene then
          ...
          if   self.gamescene:getTableConf().ttype == HPGAMETYPE.YCSDR or 
               self.gamescene:getTableConf().ttype == HPGAMETYPE.YCXZP or 
               self.gamescene:getTableConf().ttype == HPGAMETYPE.LFSDR or 
               self.gamescene:getTableConf().ttype == HPGAMETYPE.TCGZP or 
               self.gamescene:getTableConf().ttype == HPGAMETYPE.SZ96 then
               if data.piao_score and data.piao_score > 0  then
                    local tips = ccui.ImageView:create("game/result/tips_piao.png")
                    if self.gamescene:getTableConf().ttype == HPGAMETYPE.TCGZP  then
                         tips = ccui.ImageView:create("game/result/icon_pao.png")
                    end
                    ...
                    if _width == 0 then
                         ...
                         if is10dui then
                              tips:setPositionX(tips:getContentSize().width/2.0 +12)
                             _width =  - tips:getContentSize().width -5
                         end
                    end
                    tipNode:addChild(tips)
                    if is10dui then
                         _width = _width + tips:getContentSize().width/2.0 + 10
                    else
	...
	for k,v in pairs(tipsList) do 
	  local tips = ccui.ImageView:create(v.img)
		if _width == 0 then
			...
			_width =  - tips:getContentSize().width/2.0
               if is10dui then
                    tips:setPositionX(tips:getContentSize().width/2.0)
                    _width =  0
               end
		else
		...
		tipNode:addChild(tips)
          if is10dui then
               if k == #tipsList then
                    tips:setPositionX(0-(tips:getContentSize().width/2.0 + _width)-20)
               end
               _width = _width + tips:getContentSize().width/2.0 + 10
          else
		    _width = _width + tips:getContentSize().width + 10
...
function SingleResultView:getCardList(data)
	...
	local colsName =  --基本的
	{
		...
		[poker_common_pb.EN_SDR_COL_TYPE_YI_DUO_LONG] = "夺龙",
          ...
          [poker_common_pb.EN_SDR_COL_TYPE_YI_HUA] = "滑",
          [poker_common_pb.EN_SDR_COL_TYPE_YI_GANG] = "招",
          [poker_common_pb.EN_SDR_COL_TYPE_YI_AN_GANG] = "观"
	}

     if self.gamescene then  --有简单修改，直接修改的
          if self.gamescene:getTableConf().ttype == HPGAMETYPE.YCSDR then
          ...
          if self.gamescene:getTableConf().ttype == HPGAMETYPE.SJHP or  self.gamescene:getTableConf().ttype == HPGAMETYPE.WJHP then
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_ZHAO] = "招"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_CHI] = "句"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_PENG] = "对"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_AN_GANG] = "扎"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_KAN] = "坎"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_GANG] = "开泛"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_GANG_2] = "踏船"
          end
          if self.gamescene:getTableConf().ttype == HPGAMETYPE.SZ96  then
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_KAN] = "坎"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_CHI] = "吃"
          end
            
          if  self.gamescene:getTableConf().ttype == HPGAMETYPE.BDSDR then
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_TI] = "抢"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_PAO] = "卯"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_PENG] = "丁"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_CHI] = "吃"
          end

          if self.gamescene:getTableConf().ttype == HPGAMETYPE.YCXZP or
               self.gamescene:getTableConf().ttype == HPGAMETYPE.LJSDR then
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_KAN] = "坎"
               colsName[poker_common_pb.EN_SDR_COL_TYPE_YI_CHI] = "吃"
          end
          if self.gamescene:getTableConf().ttype == HPGAMETYPE.TCGZP  then
	...
	local function getColdName(data)
          if self.gamescene then
               ...
               if self.gamescene:getTableConf().ttype == HPGAMETYPE.SZ96  then
                    if data.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_ZHAO or data.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_TI or data.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_PAO then
               ...
               if self.gamescene:getTableConf().ttype == HPGAMETYPE.LJSDR then
                   if  data.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_ZHAO then
                         if data.is_waichi == true then
                              return "暗卯"
                         end
                       return "明卯"
                   end
               ...
               if  self.gamescene:getTableConf().ttype == HPGAMETYPE.YCSDR or 
                    self.gamescene:getTableConf().ttype == HPGAMETYPE.TCGZP or 
                    self.gamescene:getTableConf().ttype == HPGAMETYPE.SJHP or 
                    self.gamescene:getTableConf().ttype == HPGAMETYPE.WJHP or
                    self.gamescene:getTableConf().ttype == HPGAMETYPE.LJSDR or
                    self.gamescene:getTableConf().ttype == HPGAMETYPE.YCXZP then
	...
	-- repeated PBSDRColumnInfo col_info_lsit = 10;	// 赢家胡牌牌型
     ...
     -- print("...........赢家胡牌牌型")
     -- printTable(data.col_info_lsit,"xp80")
     ...
     if data.dest_card and data.dest_card ~= 0  then
          ...
          if data.col_info_lsit then 
               ...
               for k,v in pairs(data.col_info_lsit) do
                    ...
                    if self.gamescene and self.gamescene:getTableConf().ttype == HPGAMETYPE.YCXZP then 
                         if _data.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_PENG or
                              _data.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_ZHAO then

                              local card = _data.cards[1]
                              --上11，丘21，化31，七41，可81 是金牌，其他是素牌
                              if card ~= 0x11 and card ~= 0x21 and card ~= 0x31 and card ~= 0x41 and card ~= 0x81 then
                                   _data.score  = _data.score + 0.5
                              end    
                         end
                    end
			...
			elseif a_num == 4 then
				pai:setPositionY(15+(kk-1)*12)
                    ...
                    if kk < 5 then
                         pai:setPositionY(15+(kk-1)*12)
                    elseif kk < 10 then
                         pai:setPositionX(15+38*0.8)
                         pai:setPositionY(15+(kk-5)*20)
                    elseif kk < 15 then
                         pai:setPositionX(15+(38*0.8)*2)
                         pai:setPositionY(15+(kk-10)*20)
                    else
                         pai:setPositionX(15+(38*0.8)*3)
                         pai:setPositionY(15+(kk-15)*20)
                    end
			end
