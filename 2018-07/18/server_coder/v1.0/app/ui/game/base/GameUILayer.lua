...
function GameUILayer:initNormalBtn()
    ...
    WidgetUtils.addClickEvent(talkbtn, function()
        if device.platform ~= "ios"  and device.platform ~= "android" and device.platform ~= "mac" then
    ...
    self.tingBtn = self.normalbtn:getChildByName("tingBtn")
    if self.tingBtn then
        self.tingBtn:setPosition(backbtn:getPosition())
        self.tingBtn:setVisible(false)
        self.tingBtn:addTouchEventListener(function(sender,eventType)
...
function GameUILayer:showTingView(dest_card)
    if self.tingView == nil then

        local _tingView = cc.CSLoader:createNode("ui/game/tingBoxView.csb")
        _tingView:setPosition(cc.p(display.cx, display.cy))
        -- _tingView:addTo(self)

        glApp:getCurScene():addChild(_tingView, 100)

        local layer = _tingView:getChildByName("layer")
        WidgetUtils.addClickEvent(layer, function( )
            self.tingView:setVisible(false)
        end)

        local _main = _tingView:getChildByName("main")

        self.tingView_bg = _main:getChildByName("bg")
        self.tingView_title = _main:getChildByName("ting_icon")

        local item = _main:getChildByName("ting_info")
        item:setVisible(false)
        item:retain()
        item:removeFromParent()
        self.tingView_item = item
        self.tingView = _tingView
    end

    self.tingView:setVisible(true)

    self.tingView_bg:removeAllChildren()
    local huData = self.gamescene:getUpdataHuInfo()
    if huData == nil then
        self.tingView:setVisible(false)
        return 
    end
    ...
    if dest_card == nil then
        ...
        for k,v in pairs(huData) do
            if  dest_card == v.dest_card then
                tingData = v
            end
        end
    end

    local _wid = 118+99*(#tingData.jiao_card)
    self.tingView_bg:setContentSize(cc.size(_wid,156))
    self.tingView_title:setPositionX(-_wid/2.0+56)

    for k,v in pairs(tingData.jiao_card) do
        local _item = self.tingView_item:clone()
        _item:setVisible(true)
        self.tingView_bg:addChild(_item)
        _item:setPosition(cc.p(140+99*(k-1),78.00))

        local card = LongCard.new(CARDTYPE.ONTABLE,v)
        _item:getChildByName("addnode"):addChild(card)

        local all = 5
        if v == 0x71 or v == 0x73 or v == 0x75 or v == 0x77 or v == 0x79 then
            all = 3
        end
        if v == 0x81 or v == 0x83 or v == 0x85 or v == 0x87 or v == 0x89 then
            all = 2
        end
        print("查看牌的数量 card =",v)
        local _num = self.gamescene:getShowNum(v)
        print("查看牌的数量 _num =",_num)
        _item:getChildByName("num"):setString(all-_num)
    end
end

function GameUILayer:hiddenTingView()
    if self.tingView ~= nil then
        self.tingView:setVisible(false)
    end
end
function GameUILayer:setShowTingBtn(isShow)

    print("............GameUILayer:setShowTingBtn(isShow) = ",isShow)
    if self.tingBtn then
        self.tingBtn:setVisible(isShow)
        -- self.tingBtn:setPositionY(-50)
    end
end
