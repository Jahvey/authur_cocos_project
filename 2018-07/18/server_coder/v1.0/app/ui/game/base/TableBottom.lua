...
function TableFactory.create(tableClass, actionClass)
    ...
    function TableBottom:initLocalConf()
        ...
        if self.gamescene:getTableConf().ttype == HPGAMETYPE.SJHP or self.gamescene:getTableConf().ttype == HPGAMETYPE.WJHP then
            self.shouzhiNode:setPositionY(70*5+20)
        end
        self.outCardNode = self.node:getChildByName("chucardnode"):setVisible(false)
        if self.gamescene:getTableConf().ttype == HPGAMETYPE.SJHP or self.gamescene:getTableConf().ttype == HPGAMETYPE.WJHP then
            self.outCardNode:setPositionY(self.outCardNode:getPositionY() - 70)
        end
    end
    ...
    -- 设置捡的牌的list
    function TableBottom:setJianCardsList(list)
        -- print(debug.traceback("", 2))
        print("........设置捡牌的list")
        printTable(list,"xp25")
        self.jianList = list
    end
        ...
        for i, v in ipairs(showCardList) do
            if v.card_ti then
                ...
                if self.gamescene:getTableConf().ttype == HPGAMETYPE.LJSDR  then 
                    if v.col_type == poker_common_pb.EN_SDR_COL_TYPE_YI_ZHAO then
                        local maiNode = self:addQiShouMaiTips(v)
                        maiNode:setVisible(true)
                        maiNode:setPosition(self.nextShowPos)
                        maiNode:setLocalZOrder(100)
    ...
    function TableBottom:refreshPutoutTile()
        ...
        for i,v in ipairs(putouttile) do
            ...
            if self.gamescene:getTableConf().ttype == HPGAMETYPE.SJHP or self.gamescene:getTableConf().ttype == HPGAMETYPE.WJHP then
                if i%3 == 0 then
                    self.nextPutPos = cc.p(0-BASECARDWIDTH/2, BASECARDHEIGHT/2 + BASECARDHEIGHT*i/3)
                else
    ...
    function TableBottom:tryChupai(tile)
        if self:getIsMyTurn() and not self.actionview:getIsShow() then
            ...
            local isjian = tile:getIsJian()
            Socketapi.request_do_action(_index, _type, _card, _cards, _token,nil,isjian)
                    ...
                    ["new_sdr_action_flow"] = 
                     {
                        ["action"] =  
                        {
                            ...
                            ["act_type"] = _type,
                            ["ismyself"] = true,
                            ["colIndex_X"] = tile.colIndex_X or -1,
                            ["colIndex_Y"] = tile.colIndex_Y or -1,
                        },
                    },
                }
    ...
    function TableBottom:addMoveSprite(tile, touchPoint)
        ...
        if _paiStr then
            -- local maiType = ccui.ImageView:create("game/_xuxian.png")
            -- if maiType then
            --     maiType:setPosition(cc.p(0,0))
            --     _paiStr:addChild(maiType)
            -- end
