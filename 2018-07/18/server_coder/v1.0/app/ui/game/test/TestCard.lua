...
function TestCard:initView()
	self.widget = cc.CSLoader:createNode("ui/game/testcard.csb")
    ...
    local sendbtn = self.mainLayer:getChildByName("send")
    WidgetUtils.addClickEvent(sendbtn,function ()
        self:sendNextCard()
    end)

    -- local editbox = ccui.EditBox:create(cc.size(100,100),"common/back_tip.png")
    --     :addTo(self)
    --     :setPosition(cc.p(display.cx-80,-60))
    self.textfield = self.mainLayer:getChildByName("textfield")

    self.checkbox = self.mainLayer:getChildByName("checkbox")
end
...
function TestCard:initDeck()
    ...
    if LocalData_instance:getGameType() == HPGAMETYPE.TCGZP then
        deckcard = { 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a
        , 0x1d, 0x1e, 0x1f, 0x2, 0x2f, 0x3d, 0x3f, 0x4f, 0x5f, 0x6d, 0x6e, 0x6f
        , 0x81, 0x83, 0x85, 0x89}
    end
...
function TestCard:refreshScrollview(scrollview,typ)
	...
	for i,v in ipairs(scrollview.data) do
		local pokerCard = LongCard.new(CARDTYPE.ACTIONSHOW,v)
            :addTo(scrollview)
...
function TestCard:addCardEvent2(tile)
    ...
    local function onTouchBegan(touch, event)
        ...
        local rect = cc.rect(0, 0, s.width, s.height)
        if cc.rectContainsPoint(rect,locationInNode) then
            moveCard = LongCard.new(CARDTYPE.ACTIONSHOW, tile:getCardValue())
                :addTo(self)
                :setScale(SCALE)
...
function TestCard:readPaidui()
    local str = self:readFile("paidui")
    print("Load-----------------------",str)
    printTable(str,"xp65")
    local cards = cjson.decode(str) or {}
...
function TestCard:request(index,cards)
    ...
    print("..............庄家 = ",tonumber(self.textfield:getString()))
    if tonumber(self.textfield:getString())  then
        request.i_table_dealer_index =  tonumber(self.textfield:getString()) --庄家  
    end
        
    if  self.checkbox:isSelected() then
        for ii,vv in ipairs(self:readPaidui()) do
            request.i_cards_pile_cards:append(tonumber(vv))
        end
    end
