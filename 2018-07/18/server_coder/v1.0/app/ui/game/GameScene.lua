...
function GameScene:refreshScene()
    ...
    for i, v in ipairs(self:getSeatsInfo()) do
        print("......v.out_cards = ",v.guan_times)
        -- printTable(v.out_cards)
        if  v.index == self:getMyIndex() then
            self.mytable:setBanedCardsList(v.baned_cards or {}) --设置我不能打的牌
            self.mytable:setJianCardsList(v.jian_cards or {}) --设置我不能打的牌
    ...
    if state == poker_common_pb.EN_TABLE_STATE_PLAYING or   --玩牌阶段
        ...
        state == poker_common_pb.EN_TABLE_STATE_WAIT_PIAO or
        state == poker_common_pb.EN_TABLE_STATE_WAIT_GANG_PAI
        then 
        ...
        print(".......断线重连的奖牌 ＝ ",self.table_info.jiang_card)
        ...
        if latestaction then
            ...
            if latestaction.action.act_type == poker_common_pb.EN_SDR_ACTION_TOU then
                self.lastistou = true
                self.lastseatid = latestaction.action.seat_index
            else
...
function GameScene:waitGameStart()
    ...
    self.chuPaiIndex = -1
    self.display_anpai = false
    self.mytable:saveMyGrouping(true)
end
...
function GameScene:registerNetEventListener()
    ...
    ComNoti_instance:addEventListener("add_Debug_List_Card", self, self.addDebugList_Card)
     ...
     ComNoti_instance:addEventListener("cs_notify_hu_info", self, self.NotifyHuInfo)
...
function GameScene:NotifyHuInfo(data)
   
    if  data.seat_index == self:getMyIndex() then
        
        print("...胡牌提示胡牌提示胡牌提示..NotifyHuInfo,胡牌提示")
        printTable(data,"xp23")

        self:setUpdataHuInfo(data.check_hu_info or nil)
        -- self.mytable:refreshHandTile(false)
    end

end

function GameScene:getShowNum(card)
    local num = 0

    for i,v in ipairs(self:getSeatsInfo()) do
        if v.out_cards  then
            for kk,vv in pairs(v.out_cards) do
                if vv == card  then
                    num = num + 1
                end
            end
        ...
        if v.hand_cards then
            for kk,vv in pairs(v.hand_cards) do
                if vv == card  then
                    num = num + 1
                end
            end
...
function GameScene:NotifyActionFlow(data)
    ...
    if self:getTableConf().ttype == HPGAMETYPE.TCGZP then
        if  action.act_type == poker_common_pb.EN_SDR_ACTION_CHUPAI and 
            ...
            self.mytable:setJianCardsList(action.jian_cards or {})
            self.mytable:refreshHandTile(false)
...
function GameScene:getIsDisplayAnpai()
    print(".............是否要显示暗的牌 ", self.display_anpai)
...
function GameScene:doAction(data, _str)
    ...
    if data.act_type < 100 and data.seat_index and  data.seat_index == self:getMyIndex() and data.act_type~=poker_common_pb.EN_SDR_ACTION_PIAO then
        ...
        if self:getTableConf().ttype == HPGAMETYPE.SJHP or self:getTableConf().ttype == HPGAMETYPE.WJHP  then
            if data.act_type == poker_common_pb.EN_SDR_ACTION_OUT_CARD then
            else
                self.mytable:setBanedCardsList(data.baned_cards or {} )
            end
        else
    ...
    elseif data.act_type == ACTIONTYPEBYMYSELF.NEXT_OPERATION then
        -- 轮到谁操作，的那个闹钟的显示与倒计时
        if self:getTableConf().ttype == HPGAMETYPE.YCXZP then
            if  self.UILayer.readyNode:isVisible() == true then
                self.UILayer:gameStartAciton() 
                for i, v in ipairs(self.tablelist) do
                    v.icon:getChildByName("ok"):setVisible(false)
                end
            end
        end
    ...
    elseif data.act_type == poker_common_pb.EN_SDR_ACTION_CHUPAI then
        ...
        if data.ischong then
            ...
            self:deleteHandTile( { data.dest_card }, data.seat_index,data)
        end
    ...
    elseif data.act_type == poker_common_pb.EN_SDR_ACTION_HUPAI then
        print(".........胡牌动画")
        self:addHandTile(data.seat_index, data.dest_card)
        if data.seat_index == self.lastseatid then
    ...
    if data.act_type == poker_common_pb.EN_SDR_ACTION_TOU then
        self.lastistou = true
        self.lastseatid = data.seat_index
    end
