...
function SetView:initView_2()
	--是否有两种语言，
	if GT_INSTANCE:getIsTwoAudio(self.scene:getTableConf().ttype) then 
		self.widget = cc.CSLoader:createNode("ui/setView/setViewForGame_2.csb")
	else
	...
	if GT_INSTANCE:getIsTwoAudio(self.scene:getTableConf().ttype) then 
		self.audioList = {}
	 	local index = 1
		local box = self.mainLayer:getChildByName("language"):getChildByName("CheckBox_"..index)
		while(box) do
			table.insert(self.audioList,box)
			box:setVisible(true)
		   	index = index +1
		   	box = self.mainLayer:getChildByName("language"):getChildByName("CheckBox_"..index)
		end

		local bg_type = cc.UserDefault:getInstance():getIntegerForKey("audio_type",1)
		--1是方言，二是普通话
		for i,v in ipairs(self.audioList) do
			...
			v:addEventListener(function(sender)
				if sender:isSelected() then
					cc.UserDefault:getInstance():setIntegerForKey("audio_type",sender.index)
					for ii,vv in ipairs(self.audioList) do
						if  ii == sender.index  then
							vv:setSelected(true)
	...
	for i,v in ipairs(styleboxes) do
		...
		WidgetUtils.addClickEvent(v,function ()
			if styleboxes[i]:isSelected() then
				print("===========================点击")
