...
function ActionView:init()
    --wai  和 chi  都叫吃
    self.ACTIONCONF[poker_common_pb.EN_SDR_ACTION_CHI]= { sort = 2, img = "game/btn_jiao.png" }
  	--deng 叫 跳
  	self.ACTIONCONF[poker_common_pb.EN_SDR_ACTION_DENG]= { sort = 2, img = "game/btn_tiao.png" }
  	--wei 叫绍
  	self.ACTIONCONF[poker_common_pb.EN_SDR_ACTION_BAO] = { sort = 3, img = "game/btn_chonghu.png" }
    -- 打 
    self.ACTIONCONF[poker_common_pb.EN_SDR_ACTION_ZHAO] = { sort = 2, img = "game/btn_xu.png" }
    self.ACTIONCONF[poker_common_pb.EN_SDR_ACTION_ZHAO_2] = { sort = 2, img = "game/btn_gong.png" }
...
function ActionView:showAction(choices)
    ...
    for i,v in ipairs(choices) do
        if v.act_type == poker_common_pb.EN_SDR_ACTION_PASS then
            print("漂")
            havepass = true
        end
    end
    for i,v in ipairs(choices) do
        if v.act_type == poker_common_pb.EN_SDR_ACTION_PIAO then
            print("漂")
            self.gamescene:SelectPiao(v,havepass)
            return 
        end 
    end
