...
function MainScene:initview()
	...
	local HallView = require("app.ui.hall.HallBox").new(self)
	...
	if not SHENHEBAO then
		...
		self:runAction(cc.CallFunc:create(function()
			ComHttp.httpPOST(ComHttp.URL.MAININTERFACE,{uid = LocalData_instance.uid},function(msg)
		        
		    end)
		end))


		if device.platform == "android" and 
			cc.UserDefault:getInstance():getStringForKey("TimeLimit_Time","") ~= os.date("%m%d",os.time()) then
			print(".....LocalData_instance:getChips() = ",LocalData_instance:getChips())
			if  LocalData_instance:getChips() < 10 then
				self:runAction(cc.CallFunc:create(function()
					ComHttp.httpPOST(ComHttp.URL.TimeLimit,{uid = LocalData_instance.uid},function(msg)
						...
						LaypopManger_instance:PopBox("TimeLimitBox",msg)
						cc.UserDefault:getInstance():setStringForKey("TimeLimit_Time",os.date("%m%d",os.time()))
				    end)
				end))
			end
		end
