...
function GameScene:priorityAction(data)
    ...
    elseif data.act_type == poker_common_pb.EN_SDR_ACTION_LIANG_NIU then --买牌
        self.tablelist[data.seat_index + 1]:showniu(5)
        self:deleteAction("抓牌动画结束！")
        return true
    elseif data.act_type == poker_common_pb.EN_SDR_ACTION_NOT_LIANG_NIU then --买牌
        self:deleteAction("抓牌动画结束！")
        return true
    ...
    elseif data.act_type == poker_common_pb.EN_SDR_ACTION_CHUPAI then
        ...
        if data.over_you_index then
            self.tablelist[data.seat_index + 1]:showrank(data.over_you_index)
        end
