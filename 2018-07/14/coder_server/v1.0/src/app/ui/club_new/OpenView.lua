...
function OpenView:initView()
	...
	WidgetUtils.addClickEvent(self.createBtn, function( )
		...
		if LocalData_instance:get("roletype") == poker_common_pb.EN_Role_Type_Saler then
			...
			LaypopManger_instance:PopBox("PromptBoxView",1,{tipstr = "您不是代理,无法创建茶馆"})
		end
	end)
	...
	WidgetUtils.addClickEvent(self.joinBtn, function( )
		...
		if LaypopManger_instance:PopBox("JoinclubView",self.recommend_brief_data) then
			userdata={}
		    userdata.cid=4
		    userdata.pid=41
		    userdata.json={
...
function OpenView:enterclub(data)
	...
	if data.result == 0 then
		if data.is_refresh then
			...
			if LaypopManger_instance:PopBox("MainViewclub",data) then
...
function OpenView:msgreslutapply(data)
	...
	local str1 = "加入茶馆【"..data.tbname.."】id".."【"..data.tbid.."】失败,群主("..data.master_name..")".."拒绝了您的申请"
	local str2 = "加入茶馆【"..data.tbname.."】id".."【"..data.tbid.."】成功,群主("..data.master_name..")".."同意了您的申请"
...
function OpenView:clubejiesan(data)
	Socketapi.requestgetCluelist()
	self:showSmallLoading()
	local str = "您已经被移除茶馆【"..data.tbname.."】id".."【"..data.tbid.."】"
	LaypopManger_instance:backByName("PromptBoxView")
