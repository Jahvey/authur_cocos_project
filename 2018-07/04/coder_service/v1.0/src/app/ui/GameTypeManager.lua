...
--GameConfig[HPGAMETYPE.YCSDR]= { PATH = "game_ycsdr", 	GameName = "宜城绞胡",	style = STYLETYPE.HuaPai, 	isHelp = false,	wechatName = "宜城上大人代理详询：", 	wechat = "jrhp008"}


--GameConfig[HPGAMETYPE.YCDDZ]= { PATH = "game_ycddz", 	GameName = "应城斗地主",	style = STYLETYPE.Poker, 	isHelp = false,	wechatName = "应城斗地主代理详询：", 	wechat = "jrhp008"}
GameConfig[HPGAMETYPE.HCNG]= { PATH = "game_hcng", 	GameName = "河池牛鬼",	style = STYLETYPE.Poker, 	isHelp = true,	wechatName = "河池牛鬼代理详询：", 	wechat = "jrhp008"}
--还需要加两项，一个是入口大厅，一个是背景设置
...
function GameTypeManager:getTableDes(tableConfig, fromType)
	...
	if  fromType ~= 4 then
		if tableConfig.tbid then
			if tableConfig.tb_pay_type == 0 then
				table.insert(list,"茶馆支付")
			elseif tableConfig.tb_pay_type == 1 then
				table.insert(list,"茶馆均摊")
			end
		else
	...
	elseif tableConfig.ttype ==  HPGAMETYPE.HCNG then
		table.insert(list,"底分"..tableConfig.di_score.."分")
		if tableConfig.game_type_on_3_seat_size then
			if tableConfig.game_type_on_3_seat_size == 1 then
				table.insert(list,"牛三(无大小王)")
			elseif tableConfig.game_type_on_3_seat_size == 2 then
				table.insert(list,"双牛鬼(有大小王)")
			end
		end

		if tableConfig.shuang_niu_tou_xiang  then
			table.insert(list,"开启投降")
		else
			table.insert(list,"关闭投降")
		end
		if tableConfig.shuang_niu_liang_pai  then
			if tableConfig.shuang_niu_liang_pai == 1 then
				table.insert(list,"失败每家6分")
			elseif tableConfig.shuang_niu_liang_pai == 2 then
				table.insert(list,"失败每家4分")
			elseif tableConfig.shuang_niu_liang_pai == -1 then
				table.insert(list,"无亮牌")
			end
		end
