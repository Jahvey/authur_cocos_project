...
function Suanfucforddz:getype(data,isall)
	...
	if totall == 5 then
		...
		if istonghua then
			print("同花2")
			table.insert(cantable,{type= poker_common_pb.EN_POKER_TYPE_TONGHUA,real =realvaluetable[5],num = huase})
		end
		--顺子
			...
			for k,v in pairs(huasetab) do
				...
				if totall >= 5 then
					...
					for i=1,totall-4 do
						...
						table.insert(cantable,{type = poker_common_pb.EN_POKER_TYPE_TONGHUA,real = self:getrealvalue( values[5] ),values =values,num = k})
					end
				end
			end
